...
# add title
fig.suptitle("Analysis 2a: Mapping Vehicle Ownership to Socioeconomic Indicators",
             fontsize=14,
             fontweight='bold',
             fontname="Arial")

# create y-axis for unemployment rate
ax2 = ax1.twinx()

# create y-axis for median household income
ax3 = ax1.twinx()
...
for i, area in enumerate(df['area']):
    ...
    if area == "Area 5": 
        x -= 0.005 # so it doesn't overlap with City
    elif area == "City":
        x += 0.005 # so it doesn't overlap with Area 5
...
plt.savefig("analysis2a.png", dpi=300, bbox_inches='tight')